...
async def iq(iqthonevent):
    ...
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "𝗐𝖾𝗅𝖼𝗈𝗆𝖾 HOTD ARAB 𓃠"
...
async def _(event):
    ...
    OUTPUT = (f"♛︙  وحـدة المعلومات الخاصه بتنصيبك مع جميع الفارات  لتنصيب سورس HOTD العرب \n\n")
...
async def _(event):
    ...
    await asyncio.sleep(1)
    await edit_or_reply(event , "حسنا عزيزي جاري تحديث السورس.")
    try:
    ...
    if vname in vlist:
        ...
        if cmd == "اضف":
            if not vinfo and vname == "ALIVE_TEMPLATE":
                return await edit_delete(event, f"**♛︙  📑 يرجى متابع قناه الفارات تجدها هنا : @")
...
async def variable(var):
    ...
    if exe == "get":
        ...
        if variable in heroku_var:
            await ics.edit("**⌔ تم تغيـر** `{}` **:**\n **- المتغير :** `{}` \n**- يتم الان اعـادة تشغيـل بـوت HOTD يستغـرق الامر 2-1 دقيقـه ▬▭ ..**".format(variable, value))
        else:
...
♛  اوامر الملصقات ↢ ( .م16 )
———————×———————
شرح الأوامر : ( @cn_world ) .
قناه السورس : ( @cn_world ) .
...
———————×———————
الأمر ⦙ ( .تحديث )
الشرح :  امر لأعاده التشغيل وتحديث ملفات السورس وتسريع السورس  
———————×———————
الأمر ⦙ ( .اطفاء مؤقت + عدد الثواني )
الشرح : يقوم بأطفاء السورس  بعدد الثواني الي ضفتها  عندما تخلص الثواني سيتم اعاده تشغيل السورس  
———————×———————
الأمر ⦙ ( .الاوامر ) 
...
———————×———————
الأمر ⦙ ( .استخدامي )
الشرح :   يضهر لك كمية استخدامك لسورس HOTD
———————×———————
الأمر ⦙ ( .تاريخ التنصيب )
